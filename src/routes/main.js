...
router.post(
    ...
    [
      ...
      check('height').isNumeric().withMessage(
          'Make sure you fill with correct value (kg)'),
      check('height')
          .isInt({min : 1, max : 1000})
          .withMessage('Is it possible?'),

      check('weight').not().isEmpty().withMessage(
          'Have you fill the weight value?'),
      check('weight').isNumeric().withMessage(
          'Make sure you fill with correct value (kg)'),
      check('weight')
